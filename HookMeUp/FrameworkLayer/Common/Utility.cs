...
namespace SS.Framework.Common
{
    ...
    public static class Utility
    {
        ...
        internal static void CheckArrayParameter(ref string[] param, bool checkForNull, bool checkIfEmpty, bool checkForCommas, int maxSize, string paramName)
        {
            ...
            {
                Utility.CheckParameter(ref param[i], checkForNull, checkIfEmpty, checkForCommas, maxSize,
        ...
        public static string GetHighstPriorityPatameter(string key)
        {
            if (string.IsNullOrEmpty(key))
            {
                throw new ArgumentException("The parameter key cannot be null or empty.");
            }
            string ret = null;

            // settings in the hosting configuration are stronger than settings in app or web config
          
            
                try
                {
                  if (RoleEnvironment.IsAvailable)
                    ret = RoleEnvironment.GetConfigurationSettingValue(key);
                }
                catch (Exception)
                {
                    ret = null;
            ...
            // if setting not available try aap or web config.
        ...
        /// <returns></returns>
        public static MemoryStream readFromBlob(string blobName)
        {
            return readFromBlob("wpsprivate", blobName);
        }
        ...
        public static void UpdateBlob(string blobName, MemoryStream data)
        {
            UpdateBlob("wpsprivate", blobName, data);
        }

        /// <summary>
        /// Create or Update Blob
        /// Works with wpsprivate
        public static void UpdateBlob(string containerName, string blobName, MemoryStream data)
        {
            UpdateBlob(containerName, blobName, data, string.Empty);
        }
        ...
        public static string NumberToBaseString(int number, int nBase)
        {
            ...
            do
            {
                ...
                rest = rest / nBase;
                if (cuur < 10)
                    sb.Insert(0, cuur.ToString());
                else
                    sb.Insert(0, (char)(cuur + 87));
        ...
        public static string EncryptString(string sInputFString, byte[] sKey)
        {
             ...
             try
             {
                 ...
                 byte[] bytearrayinput = Encoding.UTF8.GetBytes(sInputFString);
                 cryptostream.Write(bytearrayinput, 0, bytearrayinput.Length);
                 cryptostream.Close();
                 rez = Convert.ToBase64String(fsEncrypted.ToArray());
                 fsEncrypted.Close();
             }
             catch (Exception ex)
             {
                 Logger.LogError("Error Encrypting string:", ex, false);
             }
            return rez;
        }

        public static string DecryptString(string sInputFString, byte[] sKey)
        {
            ...
            try
            {
                ...
                MemoryStream fsread = new MemoryStream(Convert.FromBase64String(sInputFString));
                ...
                rez =new StreamReader(cryptostreamDecr).ReadToEnd();
                //  return  Encoding.UTF8.GetString(  stri);
            }
            catch (Exception ex)
            {
                Logger.LogError("Error Encrypting string:", ex, false);
        ...
        public  static MemoryStream SerializeObject( object objectToSerialize)
        {
        ...
        public static object DeSerializeObject(MemoryStream stream)
        {
