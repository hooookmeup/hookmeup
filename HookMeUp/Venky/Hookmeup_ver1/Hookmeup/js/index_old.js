...
function onDeviceReady() {
    ...
    if (userNetworkStatus) {
        ...
        if (isUserInLocalStorage()) {
            getUserInfo(userId, {
                onDone: function () {
                    alert("inside the callback function");
                    if (userIsActive) {
                        prepareHomePage();
                    }
                    else {
...
function prepareHomePage() {
    if (isDeviceReady) {
        ...
        if (!preparePhono()) {
            navigator.notification.alert("Could not get Phono ID");
        }
        if (!updateUserInfo(userId, deviceUUID, deviceName, deviceversion, devicePlatform, phonoID)) {
            alert("saveUserInfo failed");
        }
...
function updateUserInfo(myUserId, mydeviceId, mydeviceName, mydeviceVersion, mydevicePlatform, mychatId) {
...
function onSuccess(position) {
    ...
    if (getPosition(position)) {
        ...
        if (updateUserLocation(userId, currentLatitude, currentLongitude)) {
            setLatLong(currentLatitude, currentLongitude);
        }
        else {
            navigator.notification.alert("Could not get current Location Info");
        }
    }
    else {
...
function getPosition(Pos) {
    ...
    currentTimeStamp = Pos.coords.timestamp;
    return true;
}
...
function updateUserLocation(ID, myLat, myLng) {
